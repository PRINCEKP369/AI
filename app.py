...
def get_token_usage(user_id):
    ...
    return {"used": used, "limit": TOKEN_DAILY_LIMIT, "remaining": max(0, TOKEN_DAILY_LIMIT - used)}
...
def chat_completions():
    ...
    if usage_snapshot["used"] + input_tokens > TOKEN_DAILY_LIMIT:
        return jsonify({"error": "Daily token limit reached", **usage_snapshot}), 429
    ...
    db.commit()
    max_output_tokens = max(0, TOKEN_DAILY_LIMIT - usage_snapshot["used"] - input_tokens)
